...
@njit
def cluster_search(a,b,len_a,len_b,min_length):


    skips = 0
    clusters = np.zeros((1000000,3))
    clus_pos = 0
    ...
    return clusters[0:clus_pos],clus_pos
...
def find_cluster(a, b, min_clus_length,text_a_name='text_a',text_b_name='text_b'):
    ...
    len_a = len(a)
    len_b = len(b)
    a = np.array([abs(hash(word)) % (10**8) for word in a], dtype=np.int64)
    b = np.array([abs(hash(word)) % (10**8) for word in b], dtype=np.int64)

    
    clusters, len_clus = cluster_search(a,b,len_a,len_b,min_length=min_clus_length)

    cluster_list = []
    clus_id = -1
    cluster_object = ''
    for i in range(len_clus):
        if clusters[i,0] > clus_id:
            try:
                cluster_object.pick_finalcluster()
                cluster_list.append(cluster_object)
            except:
                pass
            cluster_object = Cluster(clusters[i,0],text_a_name,text_b_name)
        cluster_object.append_cluster(clusters[i,1],clusters[i,2])
        clus_id = clusters[i,0]
    cluster_object.pick_finalcluster()
    cluster_list.append(cluster_object)
    ...
    for j in range(5):
        cluster_dict[cluster_list[0].clus_tupel_naming[j]] = []
