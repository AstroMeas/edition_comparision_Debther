# In this pyx are some of the cluster function which takes the most time optimized for cython
...
cpdef tuple cluster_length(a: List[str], b: List[str], i:int, j: int):
    """
    Determines the length of a cluster from position i in a and j in b.
    Is called in find_clusters.
    """
    cdef int length = 1  # Ein Treffer wird zu Beginn gezählt
    cdef int len_a = len(a)
    cdef int len_b = len(b)

    while i + 1 < len_a and j + 1 < len_b and a[i + 1] == b[j + 1]:
        i += 1
        j += 1
        length += 1
    return i, j, length
...
cpdef find_cluster(List[str] a,List[str] b,int min_clus_length,str text_a_name='text_a',str text_b_name='text_b'):
    """
    ...
    cdef int skips = 0  # Überspringe Indizes nach Clustertreffern
    cdef int len_a = len(a)
    cdef int len_b = len(b)
